...
def horizontal_cut_sheet(width, height, takes, states):
    S = defaultdict(list)
    S[0] = states
    ...
    for i in range(height + 1):
        ...
        for k, (area, counts, I, K) in enumerate(S[i]):
            t = get_takes(width, h, counts, takes)
            if not t:
...
def vertical_cut_sheet(width, height, takes, states):
    S = defaultdict(list)
    S[0] = states
    ...
    for i in range(width + 1):
        ...
        for k, (area, counts, I, K) in enumerate(S[i]):
            t = get_takes(w, height, counts, takes)
            if not t:
                s.append((area, counts, i, k))
                continue
...
def cut_sheet(width, height, takes, states):
    return vertical_cut_sheet(width, height, takes, states) + horizontal_cut_sheet(width, height, takes, states)
