...
def get_mask(counts):
    dst = 0
    B = 0
    for c, b in zip(counts, bits):
        dst += c << B
        B += b
    return dst


def get_counts(counts, i, c):
    return counts[:i] + (counts[i] - c,) + counts[i + 1:]


def horizontal_cut(width, height, counts, packs, k):
    S = defaultdict(list)

    while k < len(packs) and packs[k][0] == height:
        h, w, a, i, c = packs[k]
        S[h].append((a, get_counts(counts, i, c), k))
        k += 1
        ...
        if l in S:
            ...
            for a, c, k in s:
                while k < len(packs):
                    H, W, A, I, C = packs[k]
                    if W <= dw and c[I] >= C:
                        S[l + W].append((a + A, get_counts(c, I, C), k))
                    k += 1

    return area, counts


def vertical_cut(width, height, counts, packs, k):
    S = defaultdict(list)

    while k < len(packs) and packs[k][0] == width:
        w, h, a, i, c = packs[k]
        S[h].append((a, get_counts(counts, i, c), k))
        k += 1
        ...
        if t in S:
            ...
            for a, c, k in s:
                while k < len(packs):
                    W, H, A, I, C = packs[k]
                    if H <= dh and c[I] >= C:
                        S[t + H].append((a + A, get_counts(c, I, C), k))
                    k += 1
...
def cut(sheets, pieces):
    ...
    counts = tuple(q[3] for q in pieces)
    bits = [c.bit_length() for c in counts]
...
def cut_sheet(width, height, takes, states):
    ...
    for i in range(W * H):
        ...
        for k, (area, counts, I, K) in enumerate(S[i]):
            ...
            p = get_vertical_packs(w, h, t)
            if p:
                j = dh = 0
                while j < len(p):
                    if p[j][0] != dh:
                        dh = p[j][0]
                        a, c = horizontal_cut(w, dh, counts, p, j)
                        S[i + W * dh].append((area + a, c, i, k))
                    j += 1

            p = get_horizontal_packs(w, h, t)
            if p:
                j = dw = 0
                while j < len(p):
                    if p[j][0] != dw:
                        dw = p[j][0]
                        a, c = vertical_cut(dw, h, counts, p, j)
...
if __name__ == '__main__':
    ...
    # cut([(2800, 2070), (2800, 2070), (2800, 2070)], three_sheets_pieces)
    print(f'{time() - o:.2f}')
